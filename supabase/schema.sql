...
CREATE POLICY "Users can manage own cycle_settings" ON public.cycle_settings FOR ALL USING ((SELECT auth.uid()) = user_id);

-- Periods policies
DROP POLICY IF EXISTS "Users can manage own periods" ON public.periods;
CREATE POLICY "Users can manage own periods" ON public.periods FOR ALL USING ((SELECT auth.uid()) = user_id);

-- Symptoms policies
DROP POLICY IF EXISTS "Users can manage own symptoms" ON public.symptoms;
CREATE POLICY "Users can manage own symptoms" ON public.symptoms FOR ALL USING ((SELECT auth.uid()) = user_id);

-- Pain logs policies
DROP POLICY IF EXISTS "Users can manage own pain_logs" ON public.pain_logs;
CREATE POLICY "Users can manage own pain_logs" ON public.pain_logs FOR ALL USING ((SELECT auth.uid()) = user_id);

-- Mood logs policies
DROP POLICY IF EXISTS "Users can manage own mood_logs" ON public.mood_logs;
CREATE POLICY "Users can manage own mood_logs" ON public.mood_logs FOR ALL USING ((SELECT auth.uid()) = user_id);

-- Wellness logs policies
DROP POLICY IF EXISTS "Users can manage own wellness_logs" ON public.wellness_logs;
CREATE POLICY "Users can manage own wellness_logs" ON public.wellness_logs FOR ALL USING ((SELECT auth.uid()) = user_id);

-- Cravings policies
DROP POLICY IF EXISTS "Users can manage own cravings" ON public.cravings;
CREATE POLICY "Users can manage own cravings" ON public.cravings FOR ALL USING ((SELECT auth.uid()) = user_id);

-- Journal entries policies
DROP POLICY IF EXISTS "Users can manage own journal_entries" ON public.journal_entries;
CREATE POLICY "Users can manage own journal_entries" ON public.journal_entries FOR ALL USING ((SELECT auth.uid()) = user_id);

-- Preferences policies
DROP POLICY IF EXISTS "Users can manage own preferences" ON public.preferences;
CREATE POLICY "Users can manage own preferences" ON public.preferences FOR ALL USING ((SELECT auth.uid()) = user_id);
